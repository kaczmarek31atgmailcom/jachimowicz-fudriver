...
@Service
@Transactional
public class CreatePayrollDecreaseCommandHandler {
    ...
    @Autowired
    public CreatePayrollDecreaseCommandHandler(PayrollTransactionFactory payrollTransactionFactory, PersonFactory personFactory, UserService userService, UserFactory userFactory) {
    ...
    public CommandResult handle(CreatePayrollAccountUpdateCommand command) {
        payrollTransactionFactory.getDecreaseTransactionBuilder()
                ...
                .build();
        return new CommandResult(command.personId, CommandResult.Status.OK, "AccountDecreaseCreated");
    }
}
